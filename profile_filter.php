...
$fice 					  = mysql_real_escape_string(@$_POST['fice']);
$hsd 					  = mysql_real_escape_string(@$_POST['hsd']);
...
//apsc filter only 
 if($s =='true'&& $c=='true')
 	...
 	$query_APCS = "SELECT profile_filter.*, 
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.DISC LIKE '%s,c%' OR profile_filter.DISC LIKE '%c,s%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
 ...
 if($c =='true'&& $p=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.DISC LIKE '%c,p%' OR profile_filter.DISC LIKE '%p,c%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
 ...
 if($s =='true'&& $p=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.DISC LIKE '%s,p%' OR profile_filter.DISC LIKE '%p,s%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
//filter HSD only
if($hsd=='x'||$hsd=='y'|| $hsd=='z')
{
	$query_APCS = "SELECT profile_filter.*, 
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE profile_filter.XYZ ='$hsd'
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
}

//filter fice only
if($fice=='f'||$fice=='i'|| $fice=='c'||$fice=='e')
{
	$query_APCS = "SELECT profile_filter.*, 
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE profile_filter.LITE ='$fice'
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
}
...
if($f =='true'&& $s=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ LIKE '%f,s%' OR profile_filter.LEPJ  LIKE '%s,f%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
if($s =='true'&& $i=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ  LIKE '%s,i%' OR profile_filter.LEPJ  LIKE '%i,s%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
if($i =='true'&& $r=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ  LIKE '%i,r%' OR profile_filter.LEPJ  LIKE '%r,i%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
if($f =='true'&& $r=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ  LIKE '%f,r%' OR profile_filter.LEPJ  LIKE '%r,f%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
if($r =='true'&& $s=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ  LIKE '%r,s%' OR profile_filter.LEPJ  LIKE '%s,r%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
if($f =='true'&& $i=='true')
 	...
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.LEPJ  LIKE '%f,i%' OR profile_filter.LEPJ  LIKE '%i,f%')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());

}
if($s =='true'&& $c=='true'&& $hsd=='z')
 	{

 	$query_APCS = "SELECT profile_filter.*, 
 	mj_users.*
 	FROM profile_filter INNER JOIN mj_users ON profile_filter.user_id_fk =mj_users.usr_id
 	WHERE (profile_filter.DISC  LIKE '%s,c%' or profile_filter.DISC  LIKE '%c,s%' and
 	profile_filter.XYZ ='$hsd')
    GROUP BY profile_filter.user_id_fk";
    $result_APCS = mysql_query($query_APCS) or die(mysql_error());
...
<div id="content" class="">
	...
	<div id="contentContainer" >
		...
		<table width="100%" border="1" class="table table-hover table-striped">
			<tr>
				<td>Name</td>
				<td>Career Report</td>
...
<?php  while ( $rowusAPCS = mysql_fetch_object($result_APCS)){ ?>
 	<tr>
 		<td>
 			Name:     <?php echo $rowusAPCS->usr_name; ?><br/>
 			Email:    <?php echo $rowusAPCS->usr_email; ?><br/>
 			Phone No :<?php echo $rowusAPCS->usr_tel; ?>
 		</td>

 		 
 		<td><a href ="proapp/career.php?uid=<?php echo $rowusAPCS->usr_id ?>" title = "Career Report">Career Report</a></td>
 		<td><a href ="proapp/report_employer.php?uid=<?php echo $rowusAPCS->usr_id ?>">Employer Report</td>
...
<?php } ?>
						...
						<div id="usual1" class="usual" style="margin-top:20px; display:none"> 
						  <ul> 
						    <li><a href="#tab1" class="selected">Buy &amp; Sell Goods</a></li> 
						    <li><a href="#tab2">Invent &amp; Influence</a></li> 
						    <li><a href="#tab3">Identify &amp; Secure Funding</a></li> 
						  </ul> 
						  	...
						  	<div style="text-align:center; margin-top:40px" class="summary">
						  		...
						  		<div id="totalSpending" class="">
									...
									Where
						  			...
						  			?>
						  			<strong><h4>RM <?php echo number_format($rowobjectTotalSpending->TotalPledge); ?> - RM <?php echo number_format($totalCurrFailedSpending); ?></h4>
						  				<h1 style="font-size:50px; font-weight:bold; color:green">RM <?php echo number_format($totalUpSpending); ?></h1>
						  			</strong>

						  			
						  		</div><!-- /totalSpending -->
						  		<div id="" class="">
						  			<strong style="color:#aaa;">Contributed projects</strong>
						  			<?php 
									...
									Where
						  			...
						  			<td><strong style="color:green">RM<?php echo number_format($rowProject->fMoney);?></strong></td>
						  		...
						  		<div id="projectSuccess" class="">
						  			<strong style="color:#aaa;">Successful projects</strong>
						  			<?php
									...
									Where
						  			...
						  			<td><strong style="color:green">RM<?php echo number_format($rowProjectSuccess->fMoney);?></strong></td>
						  		...
						  		<div id="projectFailed" class="">
						  			<strong style="color:#aaa;">Failed projects</strong>
						  			<?php
		...
		<div id="quickToolDashboard" class="right" style="border:0px solid orange; width: 240px; padding: 5px;">
			...
			<div class="recomAction">
				...
				<a href="recruitment/sessionGateway.php" title="Recruitment" class="dashboard_color">Recruitment</a>
				<a href="#" title="Insert Free Ads" class="store_market_stall_color" id="insert-free-ads" rel="<?php echo $usr_id; ?>">Buy &amp; Sell Goods</a>
				<a href="#" title="Invent &amp; Influence" class="light_bulb_color" id="s-idea" rel="<?php echo $usr_id; ?>">Invent &amp; Influence</a>
				...
				<?php if ($totalRow_check_type != 1){ ?>
					<a href="proapp/" class="light_bulb_color">Profiling</a>
				<?php } ?>
