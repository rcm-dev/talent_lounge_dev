...
$librarycat = sqlInjectString($_GET['librarycat']);
...
Where
  mj_learn_article_category.la_cat_id = '$librarycat'";
		...
		<div class="cnscontainerPlain left">
			...
			<div id="inventcontent">
				...
				<div style="padding: 30px 0px;">
						<h3>Category &rarr; <strong><?php echo ucfirst($objectlibcatname->la_cat_name); ?></strong></h3>
						<ul class="book-ui">
							 ...
							 */
							 ...
							 $sqlRecentArticle = "SELECT
								...
								FROM
								  ...
								  mj_learn_article_category ON mj_learn_article.la_cat_id_fk =
								    mj_learn_article_category.la_cat_id
								WHERE mj_learn_article_category.la_cat_id = '$librarycat'
								ORDER BY
								  mj_learn_article.la_id DESC LIMIT 40";
							 $sqlRecentResult	 = mysql_query($sqlRecentArticle);
							 $sqlRecentResultTotalRow = mysql_num_rows($sqlRecentResult);

							 if ($sqlRecentResultTotalRow != 0) {
							 ...
							 while ($rowRecentArticle = mysql_fetch_object($sqlRecentResult)) {
								...
								echo "<br/><br/><strong>No Article in this category</strong>";
							}
...
<!-- Page Title -->
<input type="hidden" name="page_title" value="<?php echo ucfirst($objectlibcatname->la_cat_name); ?> Library " id="page_title" />
