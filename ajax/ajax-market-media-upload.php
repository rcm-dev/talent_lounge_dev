	...
	$(document).ready(function() {
	  $('#file_upload').uploadify({
		...
		'onComplete': function(event, ID, fileObj, response, data) {
			...
			$.post("ajax/ajax_market_upload_preview.php", { name: fileObj.name, ideaId: $('#ideaId').val() }, function(info) {
				//alert(info); // alert UPLOADED FILE NAME
			});
		},

	    'onAllComplete' : function(event,data) {
	      $('#mediaview').html('loading..').load('ajax/ajax-media-market-view.php?midfk='+$('#ideaId').val());
