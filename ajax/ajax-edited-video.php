	...
	$(document).ready(function() {
	  $('#file_upload_video').uploadify({
		...
		'onComplete': function(event, ID, fileObj, response, data) {
			...
			$.post("ajax/ajax_upload_vid_preview.php", { name: fileObj.name, ideaId: $('#curr_idea_edit_id').val() }, function(info) {
				//alert(info); // alert UPLOADED FILE NAME
			});
		},

	    'onAllComplete' : function(event,data) {
	      $('#mediaview').html('loading..').load('ajax/ajax-edited-vid-idea-view.php?cid='+$('#curr_idea_edit_id').val());
	    }
...
$(document).ready(function(){
	...
	$('#mediaVidview').html('loading..').load('ajax/ajax-edited-vid-idea-view.php?cid='+curr_idea_edit_id);
