	...
	$(document).ready(function() {
	  $('#file_upload').uploadify({
		...
		'onComplete': function(event, ID, fileObj, response, data) {
			...
			$.post("ajax/ajax_upload_project_preview.php", { name: fileObj.name, ideaId: $('#curr_idea_edit_id').val() }, function(info) {
				//alert(info); // alert UPLOADED FILE NAME
			});
		},

	    'onAllComplete' : function(event,data) {
	      $('#mediaview').html('loading..').load('ajax/ajax-edited-project-view.php?pid='+$('#curr_idea_edit_id').val());
	    }
...
$(document).ready(function(){
	...
	$('#mediaview').html('loading..').load('ajax/ajax-edited-project-view.php?pid='+curr_idea_edit_id);
