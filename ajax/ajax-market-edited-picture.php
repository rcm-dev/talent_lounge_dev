	...
	$(document).ready(function() {
	  $('#file_upload').uploadify({
		...
		'onComplete': function(event, ID, fileObj, response, data) {
			...
			$.post("ajax/ajax_picture_project_upload_preview.php", { name: fileObj.name, ideaId: $('#curr_idea_edit_id').val() }, function(info) {
				//alert(info); // alert UPLOADED FILE NAME
			});
		},

	    'onAllComplete' : function(event,data) {
	      $('#mediaview').html('loading..').load('ajax/ajax-edited-market-uploaded.php?mid='+$('#curr_idea_edit_id').val());
	    }
...
<form action="upload_preview.php" method="post" accept-charset="utf-8">
	...
	<input type="hidden" name="curr_idea_edit_id" id="curr_idea_edit_id" value="<?php echo $_GET['mid']; ?>">
...
$(document).ready(function(){
	...
	$('#mediaview').html('loading..').load('ajax/ajax-edited-market-view.php?mid='+curr_idea_edit_id);
