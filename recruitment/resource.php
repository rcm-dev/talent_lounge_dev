	...
	<section id="middle">
		  ...
		  <div id="content" class="search_container" style="width:610px; padding-top:10px;margin-top:30px;">
	        ...
	        <div id="article_section">
  			      <strong><?php echo $row_rsArtCat['ac_namr']; ?> &raquo;</strong><br/><br/>
		        <table width="600px" border="0" cellspacing="2" cellpadding="2">
                <?php
				
				$sqlArticleCat = "SELECT * FROM jp_article WHERE arc_published = 1 AND art_cat_id_fk = ".$row_rsArtCat['ac_id'];
				$sqlArticleCatResult = mysql_query($sqlArticleCat);
				$sqlArticleCatResultRow = mysql_num_rows($sqlArticleCatResult);
				
				if($sqlArticleCatResultRow != 0) {
				
				?>
                <?php
				
				while($sqlArticleCatRowObject = mysql_fetch_object($sqlArticleCatResult)){
				?>
		          <tr>
		            <td width="350"><a href="content-details.php?cid=<?php echo $sqlArticleCatRowObject->art_id; ?>"><?php echo $sqlArticleCatRowObject->art_title; ?></a></td>
		            <td width="50">&nbsp;</td>
		            <td>&nbsp;</td>
                   </tr>
                 <?php } ?>
                 ...
                 <?php } else { ?>
                 	No content at this category.
                 <?php } ?>
  </table>
  <br/><br/>
	          </div>
  			    <?php } while ($row_rsArtCat = mysql_fetch_assoc($rsArtCat)); ?>
          </div><!-- #content-->
