...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO mj_videoResume (id_videoResume, video_path,user_id_fk, video_upload_on) VALUES (%s, %s, %s, NOW())",
                       GetSQLValueString($_POST['id_videoResume'], "int"),
                       GetSQLValueString($_FILES['video_path']['name'], "text"),
                       GetSQLValueString($_POST['user_id_fk'], "int"),
                       GetSQLValueString($timestamp ."-". str_replace(" ", "-", $_FILES['video_path']['name']), "text"));
  ...
  // move uploaded file
  if($Result1) {
    move_uploaded_file($_FILES["video_path"]["tmp_name"], "media/video_candidate/" . $timestamp . "-" . str_replace(" ", "-", $_FILES["video_path"]["name"]));
  }
...
<div class="master_details">
  ...
  <p>&nbsp;</p>
   ...
   <tr>
     <td>
       <img src="../images/flat/64/file_type_video.png" alt="">
     </td>
     <td>
       <h3>File Resume</h3>
       <p>Only mov,flv, wmv, mp4, avi</p>
     </td>
   </tr> 
  </table>
  <p>&nbsp;</p>
    <form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1">
      <table align="center">
        <tr valign="baseline">
          <td nowrap align="right">Video File</td>
          <td><label for="video_path"></label>
            <input type="file" name="video_path" id="video_path"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">&nbsp;</td>
          <td><input type="submit" id="submitResume" class="btn btn-success" value="Upload Video"></td>
        </tr>
      </table>
      <input type="hidden" name="id_videoResume" value="">
      ...
      <input type="hidden" name="video_upload_on" value="">
...
$(document).ready(function() {
    ...
    if($('input#resume_path').val() == "") {
      ...
      if($.inArray(ext, ['mov','flv','wmv', 'mp4', 'avi']) == -1) {
        alert('invalid extension! Only mov,flv, wmv, mp4, avi!');
        return false;
      }
