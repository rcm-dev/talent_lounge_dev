...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE jp_users SET users_email=%s, users_pass=%s WHERE users_id=%s",
                       GetSQLValueString($_POST['users_email'], "text"),
                       GetSQLValueString(md5($_POST['users_pass']), "text"),
                       GetSQLValueString($_POST['users_id'], "int"));

  mysql_select_db($database_conJobsPerak, $conJobsPerak);
  $Result1 = mysql_query($updateSQL, $conJobsPerak) or die(mysql_error());

  $updateGoTo = "jobSeekerChangePasswordSucces.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$totalRows_rsProfile = "-1";
if (isset($_GET['email'])) {
  $totalRows_rsProfile = $_GET['email'];
}
$colname_rsProfile = "-1";
if (isset($_GET['email'])) {
  ...
  <?php include("jobSeekerMenu.php"); ?>
    ...
    <div class="box">
        ...
        <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
          ...
          <input type="hidden" name="users_email" value="<?php echo $_SESSION['MM_Username']; ?>">
...
$(document).ready(function(){
	$('#submitNewPass').live('click', function(){
