...
/****************************
  *
  * Record Set for AGentID 
  * MySQL Info 
  * Table Used AGentID
  *
  ***************************/
 
 $query_rsAGentID = "SELECT * FROM recruit_profile WHERE user_id_fk = " . $_SESSION['MM_UserID'];
 $result_rsAGentID = mysql_query($query_rsAGentID);
 $rows_rsAGentID = mysql_fetch_object($result_rsAGentID);
...
/****************************
 *
 * Record Set for ProjectLists 
 * MySQL Info 
 * Table Used ProjectLists
 *
 ***************************/

$query_rsProjectLists = "SELECT * FROM recruit_apointed WHERE ra_status = 1 AND ra_recruit_id_fk = " . $rows_rsAGentID->rp_id;
$result_rsProjectLists = mysql_query($query_rsProjectLists);
$total_rows_rsProjectLists = mysql_num_rows($result_rsProjectLists);


?>
<div style="width:600px;">
  <h2>Assign this Jobseeker to project</h2>
  <div id="mainControllerAssignProject">
  <br>
  <?php if ($total_rows_rsProjectLists != 0): ?>
    <form action="#" id="assignControlForm">
      <table>
        <tr>
          <td>
            <select name="assign_to_project" id="assign_to_project" style="padding:15px; font-size:18px; width:250px;">
              <option value="0">Choose project</option>
                <?php while ($row_rsProjectLists = mysql_fetch_array($result_rsProjectLists)) { ?>
                  <option value="<?php echo $row_rsProjectLists['ra_id'] ?>"><?php echo $row_rsProjectLists['ra_title'] ?></option>
                <?php } ?>
              </select>
              <input type="hidden" name="js_id" value="<?php echo mysql_real_escape_string($_GET['js_id']); ?>">
              <input type="hidden" name="agent_id" value="<?php echo $rows_rsAGentID->rp_id ?>"><br>
            </td>
            <td>
              <button class="button green" id="assignProjectBtn">Assign into project now!</button>
            </td>
        </tr>
      </table>
    </form>  
  ...
  <div class="successMessage" style="color:green; font-weight:bold; display:none">
    <p>Project Assigned! Go to My Project to request Interview.</p>
  </div>
...
<?php if ($totalRows_rsJobSeekerCheck > 0) { // Show if recordset not empty ?>
  ...
  <?php } // Show if recordset not empty ?>
    ...
    $(document).ready(function() {
      ...
      $('button#assignProjectBtn').click(function(){

        var form_assign = $('form#assignControlForm');
        var serializeData = form_assign.serialize();


        $.ajax({
          url: 'ajax/assignProjectAgent.php?'+serializeData,

          success:function(data){
            if (data == 1) {
              $('#mainControllerAssignProject').hide();
              $('.successMessage').show();
            };
            console.log(data);
          }
