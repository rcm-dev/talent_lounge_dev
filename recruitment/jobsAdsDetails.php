...
mysql_select_db($database_conJobsPerak, $conJobsPerak);
$query_rsEmployerDetect = sprintf("SELECT * FROM mj_users WHERE users_id = %s", GetSQLValueString($colname_rsEmployerDetect, "int"));
...
/****************************
 *
 * Record Set for PlusOneAdsView 
 * MySQL Info 
 * Table Used PlusOneAdsView
 *
 ***************************/

$query_rsPlusOneAdsView = "UPDATE jp_ads SET ads_view = ads_view+1 WHERE ads_id = " . mysql_real_escape_string($_GET['jobAdsId']);
$result_rsPlusOneAdsView = mysql_query($query_rsPlusOneAdsView);
...
<body>
  ...
  <div class="master_details">
    <p><h1 class="rTitle"><?php echo $row_rsJobsAdsDetails['ads_title']; ?></h1></p>
...
<p><strong>Office Tour</strong>
  ...
  <div class="apply_container">
   ...
   // if xlogin display 
   if (!isset($_SESSION['MM_UserID'])) { ?>
            <a href="../login.php" id="loginFromJob" class="fancybox tl-btn-blue">Login to Apply</a>
<?php 
	
	} else { 
	
		// check if view is employer
		$sql_isEmployer = "SELECT * FROM mj_users WHERE users_id = ".$_SESSION['MM_UserID'];
  ...
  <table align="center">
    <tr valign="baseline">
      ...
      <td><input type="submit" class="btn btn-danger" value="APPLY THIS JOB">
        </td>
      </tr>
    </table>
...
<div class="jobEmployer"><br/>
  ...
  <div class="left" style="width:120px; height:120px; margin-right:5px;">
  	<a href="employer.php?emp_id=<?php echo $row_rsJobsAdsDetails['emp_id_fk']; ?>&employer=<?php echo $row_rsJobsAdsDetails['emp_name']; ?>">
