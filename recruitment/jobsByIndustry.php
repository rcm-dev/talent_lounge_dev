...
mysql_select_db($database_conJobsPerak, $conJobsPerak);
$query_rsJobByIndustry = sprintf("Select   jp_industry.indus_name,   jp_employer.*,   jp_ads.*,   jp_location.location_name,   jp_ads.ads_industry_id_fk As ads_industry_id_fk1 From   jp_ads Inner Join   jp_industry On jp_ads.ads_industry_id_fk = jp_industry.indus_id Inner Join   jp_employer On jp_ads.emp_id_fk = jp_employer.emp_id Inner Join   jp_location On jp_ads.ads_location = jp_location.location_id Where   jp_ads.ads_industry_id_fk = %s And   jp_ads.ads_enable_view = 1", GetSQLValueString($colname_rsJobByIndustry, "int"));
...
<body>
  ...
  <ul id="tl-joblist">
    <?php do { ?>
      <li>
        <div style="float:left; 400px;">
          <div><a href="jobsAdsDetails.php?jobAdsId=<?php echo $row_rsJobByIndustry['ads_id']; ?>"><?php echo ucfirst($row_rsJobByIndustry['ads_title']); ?></a> <br/><small><a href="jobsByIndustry.php?ads_industry_id_fk=<?php echo $row_rsJobByIndustry['ads_industry_id_fk']; ?>&industry=<?php echo $row_rsJobByIndustry['indus_name']; ?>"><?php echo $row_rsJobByIndustry['indus_name']; ?></a></small></div>
        <div><a href="employer.php?emp_id=<?php echo $row_rsJobByIndustry['emp_id_fk']; ?>&employer=<?php echo $row_rsJobByIndustry['emp_name']; ?>"><?php echo $row_rsJobByIndustry['emp_name']; ?></a></div>
        <div>Salary: RM <?php echo $row_rsJobByIndustry['ads_salary']; ?></div>
        <div>Experience: <?php echo $row_rsJobByIndustry['ads_y_exp']; ?> year(s)</div>
        </div>
        <div style="float:right; width:160px; height:100px; text-align: center;">
          <img src="media/employer/img/<?php echo $row_rsJobByIndustry['emp_pic']; ?>" alt="<?php echo $row_rsJobByIndustry['emp_name']; ?>" width="100px">
        </div>
