...
<div class="master_details_full">
  ...
  <?php if ($row_rsIsActive['user_active'] != 0){ ?>
  <br/>
  <h3>Recruiter Appointers</h3>
  <p>
    List of recruiters that has been appointed.
  </p>


  <div id="appointerContainer">
    <div class="left" id="appointerLists">
      <table class="csstable2" width="100%" id="appointerListsClicked">
        <tr>
          <th>
        ...
        /****************************
         *
         * Record Set for AgentListByEmployer 
         * MySQL Info 
         * Table Used AgentListByEmployer
         *
         ***************************/
        
        $query_rsAgentListByEmployer = "SELECT * FROM recruit_apointed
                                            INNER JOIN recruit_profile
                                            ON recruit_apointed.ra_recruit_id_fk = recruit_profile.rp_id
                                          WHERE ra_emp_id_fk = " . $_SESSION['MM_UserID'] . " ORDER BY ra_status = 1 DESC";
        $result_rsAgentListByEmployer = mysql_query($query_rsAgentListByEmployer);
        $total_rows_rsAgentListByEmployer = mysql_num_rows($result_rsAgentListByEmployer);

        ?>
        <?php if ($total_rows_rsAgentListByEmployer == 0): ?>
        <tr>
          <td>No appointed. <strong><a href="findRecruiter.php">Find a Recruiter &rarr;</a></strong></td>
        </tr>  
        <?php endif ?>
        
        <?php if ($total_rows_rsAgentListByEmployer != 0): ?>
          
          <?php while ($row_rsAgentListsEmployer = mysql_fetch_object($result_rsAgentListByEmployer)) { ?>
            <tr class="clickableAgent" data-id="<?php echo $row_rsAgentListsEmployer->rp_id ?>" data-role="<?php echo $row_rsAgentListsEmployer->ra_title ?>" data-loc="<?php echo $row_rsAgentListsEmployer->ra_location ?>" data-pid="<?php echo $row_rsAgentListsEmployer->ra_id ?>">
              <td>
                <div class="left" style="width:60px;">
                  <img src="../../uploads/<?php echo $row_rsAgentListsEmployer->rp_profile_pic_thumb ?>" alt="<?php echo $row_rsAgentListsEmployer->rp_full_name ?>" width="60px">
                </div>
                <div class="right" style="width:220px">
                  <div class="statusAgent">
                    <?php  

                    switch ($row_rsAgentListsEmployer->ra_status) {
                      case 1:
                        $status = "Approved";
                        break;
                      case 2:
                    ...
                    ?>
                    <div class="statusAgent<?php echo $row_rsAgentListsEmployer->ra_status ?>">
                      <?php echo $status ?>
                    </div>
                  </div>
                  <strong><?php echo $row_rsAgentListsEmployer->rp_full_name ?></strong><br>
                  <ul id="agentListOption">
                    <li>
                      <img src="../images/icon_color/user-business.png" alt=""> <?php echo $row_rsAgentListsEmployer->ra_title ?>
                    </li>
                    <li>
                      <img src="../images/icon_color/marker.png" alt=""> <?php echo $row_rsAgentListsEmployer->ra_location ?>
                    </li>
                  </ul>
                </div>
                <div class="clear"></div>
    ...
    <div class="right" id="appointerResumeContainer">
      <table class="csstable2" width="100%">
        <tr>
          <th colspan="2">Featured Resume</th>
        </tr>
        <?php if ($total_rows_rsAgentListByEmployer == 0): ?>
        <tr>
          <td>No appointed. <strong><a href="findRecruiter.php">Find a Recruiter &rarr;</a></strong></td>
        ...
        <?php if ($total_rows_rsAgentListByEmployer != 0): ?>
         ...
         <tr style="background:white;">
          <td coslspan="2" id="featuredResumeContainer">
            Click agent on left to view featured resume.
          </td>
        </tr> 
  ...
  $(document).ready(function() {
    ...
    $('tr.clickableAgent').click(function(){

      var rp_id = $(this).attr('data-id');
      var role = $(this).attr('data-role');
      var pid = $(this).attr('data-pid');
      var location = $(this).attr('data-loc');

      var parameter = 'rp_id='+rp_id+'&role='+role+'&location='+location+'&pid='+pid;

      $.ajax({
        url: 'ajax/displayFeaturedResumeFromRecruiter.php?'+parameter,

        success:function(data){

          $('td#featuredResumeContainer').html(data);
          // console.log(data);
        }
      ...
      console.log('clicked view ' + pid);

      $('a.empViewResume').click(function(){
        return false;
      });
