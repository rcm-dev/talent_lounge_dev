...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("UPDATE jp_jobseeker SET jobseeker_pic=%s WHERE users_id_fk=%s",
					   GetSQLValueString('media/jobseeker/'.$timestamp ."-". str_replace(" ", "-", $_FILES['resume_path']['name']), "text"),
  ...
  // move uploaded file
  if($Result1) {
	  move_uploaded_file($_FILES["resume_path"]["tmp_name"], "media/jobseeker/" . $timestamp . "-" . str_replace(" ", "-", $_FILES["resume_path"]["name"]));
  }

  $insertGoTo = "jobSeekerMyResume.php?email=".$_SESSION['MM_Username'];
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="master_details">
  ...
  <div class="master_details boxcenter">
	<h3>Upload Profile Picture</h3><br/>
	<p>&nbsp;</p>
    <form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1">
      <table align="center">
        <tr valign="baseline">
          <td nowrap align="right">Picture file:</td>
          <td><label for="resume_path"></label>
            <input type="file" name="resume_path" id="resume_path"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">&nbsp;</td>
          <td><input type="submit" id="submitResume" value="Upload Picture"></td>
        </tr>
      </table>
